...
def validate_from_cli(opts):
    ...
    if opts.subparser == "check-no-new-access":
        checker = iamcheck_AccessAnalyzer.Comparator(
            account.Region,
            json.dumps(_load_json_file(opts.reference_policy)),
            opts.reference_policy_type,
        )
        checker.run(plan)
        ...
        if opts.actions is None and opts.resources is None:
            raise ApplicationError(
                "ERROR: At least one of --actions or --resources must be specified."
            )
        else:
            checker = iamcheck_AccessAnalyzer.AccessChecker(
                account.Region, opts.actions, opts.resources
            )
            checker.run(plan)
            findings = checker.findings
    ...
    treat_as_blocking = (
        ...
        if "treat_as_blocking" in vars(opts)
        else default_finding_types_that_are_blocking
    )
    allowed_external_principals = (
        opts.allowed_external_principals
        if "allowed_external_principals" in vars(opts)
        else None
    )
    reporter = Reporter(
        opts.ignore_finding, treat_as_blocking, allowed_external_principals
    )
...
def cli_parse_opts():
    ...
    parser = argparse.ArgumentParser(
        description="Parses IAM identity-based and resource-based policies from Terraform templates."
    )
    ...
    validate_parser = subparsers.add_parser(
        "validate",
        help="Parses IAM identity-based and resource-based policies from Terraform templates "
    ...
    def add_policy_analysis_subparsers():
        ...
        compare_parser = subparsers.add_parser(
            "check-no-new-access",
            help="Parses IAM identity-based and resource-based policies from AWS Terraform templates "
        ...
        compare_parser.add_argument(
            ...
            dest="reference_policy_type",
            required=True,
            type=str,
            help="The type of the reference policy (identity or resource)",
        )
        ...
        check_access_parser = subparsers.add_parser(
            "check-access-not-granted",
            help="Parses IAM identity-based and resource-based policies from AWS CloudFormation "
            "templates and runs them through IAM Access Analyzer to check that access to a list of actions and/or "
            "resources is not granted. Returns the response in JSON format.",
            parents=[parent_parser],
        )

        check_access_parser.add_argument(
            "--resources",
            dest="resources",
            ...
            "At least one of --actions or --resources must be specified.",
            action=ParseListFromCLI,
        )
        ...
        check_no_public_access_parser = subparsers.add_parser(
            "check-no-public-access",
            help="Parses resource-based policies from AWS Terraform templates and runs them through "
            "IAM Access Analyzer to check that public access to resources of supported types is not granted.  Returns the response "
            "in JSON format.",
            parents=[parent_parser],
        )

        check_no_public_access_parser.add_argument(
            "--ignore-finding",
            dest="ignore_finding",
            metavar="FINDING_CODE,RESOURCE_NAME,RESOURCE_NAME.FINDING_CODE",
            help="Allow findings to be ignored.\n"

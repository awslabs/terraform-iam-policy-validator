...
class Reporter:
    ...
    def __init__(
        self,
        findings_to_ignore,
        finding_types_that_are_blocking,
    ...
    def _is_finding_ignored(self, finding):
        ...
        is_ignored = any(
            [
                finding_to_ignore.matches(finding)
                for finding_to_ignore in self.findings_to_ignore
            ]
        )
        if is_ignored:
            return True
    ...
    def _is_external_principal_allowed(self, finding):
        ...
        is_allowed = any(
            [
                principal_to_allow.matches(finding)
                for principal_to_allow in self.allowed_external_principals
            ]
        )
        if is_allowed:
            return True
...
class ResourceOrCodeFindingToIgnore:
    ...
    def matches(self, finding):
        return (
            finding.resourceName.lower() == self.value.lower()
            or finding.code.lower() == self.value.lower()
        )
...
class ResourceAndCodeFindingToIgnore:
    ...
    def matches(self, finding):
        return (
            finding.resourceName.lower() == self.resource_name.lower()
            and finding.code.lower() == self.code.lower()
        )
    ...
    def __eq__(self, other):
        ...
        return self.resource_name == other.resource_name and self.code == other.code
...
class Report:
    ...
    def to_json(self):
        return {
            ...
            "NonBlockingFindings": [
                vars(finding) for finding in self.nonblocking_findings
            ],
        }

...
class TerraformPlan:
    ...
    def getResourceName(self, ref):
        ...
        if resource.type not in config.arnServiceMap:
            raise TypeError(
                f"Add resource type {resource.type} in the configuration arnServiceMap"
            )
            ...
            if state is strFsm.parseString:
                ...
                if char == interpolation[2]:
                    ...
                    try:
                        ...
                        LOGGER.warning(
                            f"string interpolation not available: {expression}"
                        )
                ...
                raise RuntimeError(
                    f"FSM for parsing HCL string reached an unknown state: {state}"
                )
        return result
